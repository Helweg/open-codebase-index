...
- **Workspace package call graph resolution**: TypeScript and JavaScript call edges now follow project-local package imports selected by root `package.json` workspace array or object patterns, including exclusions and exact source-facing `exports` entries and safe package-relative subpaths. Discovery remains bounded to known project sources, excludes `node_modules` and outside-project paths, preserves ancestor compatibility when no workspace declaration exists, and refreshes graph edges when root ownership or selected manifests change. External dependencies, unsafe metadata, and duplicate package names remain unresolved rather than being guessed.
