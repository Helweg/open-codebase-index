...
- **Name-based call graph routing**: `call_graph` now resolves callers and callees from human-readable symbol names across OpenCode, MCP, and Pi, reports bounded same-name ambiguity with file qualifiers, preserves unresolved caller references alongside selected resolved definitions, caps public edge output with omitted counts, and keeps internal symbol IDs out of agent-facing output. `call_graph_path` now offers endpoint file/directory qualifiers and uses ID-constrained traversal instead of silently mixing same-name definitions.
