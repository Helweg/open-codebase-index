...
- **Portable `code_communities` tool**: Added a new graph analysis tool that exposes community detection and hub node analysis across OpenCode, MCP, and Pi. Clusters symbols by call-graph connectivity using label propagation, reports community memberships with member listings, and identifies hub symbols with cross-community connections. Supports `branch`, `minSize`, `limit`, and `hubThreshold` parameters. Recomputes on-demand from current branch data for freshness without persisted state.
